...
type StatusProjeto = "Rascunho" | "Aguardando Aprovação" | "Recusado" | "Em Desenvolvimento" | "Submetido" | "Avaliado";
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3000/users/alunos', { headers })
      .then(r => r.json())
      .then(data => setAlunosDisponiveis(
        data
          .filter((a: any) => String(a.id) !== localStorage.getItem('userId'))
          .map((a: any) => ({ id: String(a.id), nome: a.nome, sala: '' }))
      ));

    fetch('http://localhost:3000/users/orientadores', { headers })
      .then(r => r.json())
      .then(data => setOrientadoresDisponiveis(
        data.map((o: any) => ({ id: String(o.id), nome: o.nome, disciplina: o.email_institucional }))
      ));
  }, []);
              ...
              <section>
                ...
                <div className="relative">
                  <select required value={orientadorId} onChange={(e) => setOrientadorId(e.target.value)}
                    ...
                    {orientadoresDisponiveis.map((o) => ( // 👈 dados reais
