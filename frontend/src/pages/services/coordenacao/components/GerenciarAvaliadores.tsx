...
import { API_BASE_URL } from "../../../../lib/api"; // Ajuste o caminho se necessário
...
export default function GerenciarAvaliadores() {
  ...
  const carregarAvaliadores = async () => {
    ...
    try {
      // Endpoint 1: Listar avaliadores (Ajuste a rota conforme seu backend)
  ...
  useEffect(() => {
    // Implementar um debounce aqui seria o ideal em produção para não floodar o backend
  ...
  // --- ABRIR MODAL E CARREGAR PROJETOS ---
  const abrirModal = async (avaliador: Avaliador) => {
    ...
    try {
      // Busca os projetos que o avaliador já tem e os disponíveis
      // Substitua pelas suas rotas reais
      const [resAtuais, resDisponiveis] = await Promise.all([
        fetch(`${API_BASE_URL}/avaliadores/${avaliador.id}/projetos`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API_BASE_URL}/projetos?disponiveis=true`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const fecharModal = () => {
    ...
    carregarAvaliadores(); // Recarrega a lista principal para atualizar as cotas
  };
  ...
  const toggleCheck = (id: number, state: number[], setState: React.Dispatch<React.SetStateAction<number[]>>) => {
    state.includes(id) ? setState(state.filter(item => item !== id)) : setState([...state, id]);
  };
